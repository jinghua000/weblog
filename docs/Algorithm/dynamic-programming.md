...
**一个有10层的楼梯，每次可以向上走1步或者2步，一共有多少种走法可以走到。**
...
完成了，这样时间复杂度就降为了`O(n)`，如果是10阶台阶，我们也只要计算10次就好了。
...
好了，因为一共只使用了3个变量，这样一来我们的空间复杂度便下降成了`O(1)`。最终以时间复杂度`O(n)`，空间复杂度`O(1)`完成了这个问题。
...
> 到前一家为止偷到的最大金额（注意这个概念，并不是前一家的金额），或者到前两家为止偷到的最大金额加上本家金额之和，取两者中比较大的一个。

能想到这个应该是最关键的一点，动态规划经常需要使用比较抽象的概念把前后状态连接起来。

而第一间房子能偷到的最大金额肯定就等于第一间房子的金额，结合上面的公式代码实现如下
...
这样一来就完成了时间复杂度`O(n)`的计算，然后因为整体一共只引入了3个状态量，所以可以变成仅引用3个变量。
...
像上面这种优化到最后空间复杂度为`O(1)`，然后就完成了这道问题，可喜可贺。

## 总结

上面两道例题分别展示了最基本的，以及稍微有一点点抽象的动态规划的思路，说到底，如果一个问题能够**从小问题有规律的转换过来**，就可以使用动态规划。如果能合理使用动态规划，将会对复杂度有较大的降低。

上面两道例子均在`LeetCode`寻找的，另外这里还有一些[附加题](./dynamic-programming-ext.md)。

## 参考

- [题目1](https://leetcode-cn.com/problems/climbing-stairs/)
- [题目2](https://leetcode-cn.com/problems/house-robber/)
